...
public class LabratProxyIT
    extends LabratITSupport
{
  ...
  private LabratClient proxyClient;
  ...
  public static Option[] configureNexus() {
    return NexusPaxExamSupport.options(
        ...
        nexusFeature("org.sonatype.nexus.plugins", "labrat")
    );
  }
  ...
  public void unresponsiveRemoteProduces404() throws Exception {
    Server server = Server.withPort(0).serve("/*")
        ...
        .start();
    try {
      proxyRepo = repos.createLabratProxy("labrat-test-proxy-notfound", server.getUrl().toExternalForm());
      proxyClient = labratClient(proxyRepo);
  ...
  @Test
  public void retrieveLabratWhenRemoteOffline() throws Exception {
    Server server = Server.withPort(0).serve("/*")
        .withBehaviours(content("Response"))
        .start();
    try {
      proxyRepo = repos.createLabratProxy("labrat-test-proxy-offline", server.getUrl().toExternalForm());
      proxyClient = labratClient(proxyRepo);
