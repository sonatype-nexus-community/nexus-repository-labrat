...
-->
<!-- generated with nexus-format-archetype version 1.0.30 on May 4, 2020 -->
# Nexus Repository Labrat Format

[![Maven Central](https://img.shields.io/maven-central/v/org.sonatype.nexus.plugins/labrat.svg?label=Maven%20Central)](https://search.maven.org/search?q=g:%22org.sonatype.nexus.plugins%22%20AND%20a:%22labrat%22) [![CircleCI](https://circleci.com/gh/sonatype-nexus-community/labrat.svg?style=shield)](https://circleci.com/gh/sonatype-nexus-community/labrat) [![Join the chat at https://gitter.im/sonatype/nexus-developers](https://badges.gitter.im/sonatype/nexus-developers.svg)](https://gitter.im/sonatype/nexus-developers?utm_source=badge&utm_medium=badge&utm_campaign=pr-badge&utm_content=badge) [![DepShield Badge](https://depshield.sonatype.org/badges/sonatype-nexus-community/labrat/depshield.svg)](https://depshield.github.io)
...
* [Developing](#developing)
   ...
   * [Building](#building)
* [Using Labrat with Nexus Repository Manager 3](#using-labrat-with-nexus-repository-manager-3)
...
* [Features Implemented In This Plugin](#features-implemented-in-this-plugin)
   * [Supported Labrat Commands](#supported-labrat-commands)
...
If everything checks out, the bundle for labrat should be available in the `target` folder
...
#### Run as a Docker container
    ...
    docker run -d -p 8081:8081 --name labrat labrat 
...
To read the generated admin password for your first login to the web UI, you can use the command below against the running docker container:
    ...
    docker exec -it labrat cat /nexus-data/admin.password && echo

For simplicity, you should check `Enable anonymous access` in the prompts following your first login.   

## Using Labrat With Nexus Repository Manager 3

[We have detailed instructions on how to get started here!](docs/labrat_user_documentation.md)
...
All released versions can be found [here](https://github.com/sonatype-nexus-community/labrat/releases).
...
### Supported Labrat Commands

#### Proxy

| Plugin Version               | Nexus Repository Version |
|------------------------------|--------------------------|
| `labrat client command`         | :heavy_check_mark:       |

## Installing the plugin

There are a range of options for installing the labrat plugin. You'll need to build it first, and
then install the plugin with the options shown below:

### Easiest Install

Thanks to some upstream work in Nexus Repository, it's become a LOT easier to install a plugin. To install the `labrat` plugin, follow these steps:

* Build the plugin with `mvn clean package -PbuildKar`
* Copy the `labrat-0.0.1-bundle.kar` file from your `target` folder to the `deploy` folder for your Nexus Repository installation.

Once you've done this, go ahead and either restart Nexus Repo, or go ahead and start it if it wasn't running to begin with.

You should see `labrat (proxy)` in the available Repository Recipes to use, if all has gone according to plan :)
...
* Run Nexus Repo console:
  ...
  $ cd <nexus_dir>/bin
  $ ./nexus run
  > bundle:install file:///tmp/labrat-0.0.1.jar
  > bundle:list
  ```
  (look for org.sonatype.nexus.plugins:labrat ID, should be the last one)
  ```
  > bundle:start <org.sonatype.nexus.plugins:labrat ID>
  ```

### (more) Permanent Install

For more permanent installs of the labrat plugin, follow these instructions:

* Copy the bundle (labrat-0.0.1.jar) into <nexus_dir>/deploy
...
If you are trying to use the labrat plugin permanently, it likely makes more sense to do the following:

* Copy the bundle into `<nexus_dir>/system/org/sonatype/nexus/plugins/labrat/0.0.1/labrat-0.0.1.jar`
* Make the following additions marked with + to `<nexus_dir>/system/org/sonatype/nexus/assemblies/nexus-core-feature/3.x.y/nexus-core-feature-3.x.y-features.xml`

   ```
         <feature prerequisite="false" dependency="false">wrap</feature>
   +     <feature prerequisite="false" dependency="false">labrat</feature>
   ```
   ...
   And
   ```
   + <feature name="labrat" description="org.sonatype.nexus.plugins:labrat" version="0.0.1">
   +     <details>org.sonatype.nexus.plugins:labrat</details>
   +     <bundle>mvn:org.sonatype.nexus.plugins/labrat/0.0.1</bundle>
   + </feature>
    </features>
   ```
   as the last feature.
...
* Do NOT file Sonatype support tickets related to Labrat support in regard to this plugin
...
The project has a “format” module, and an “IT” module. 
   ...
   In this project, the sub module: [labrat](labrat) is the "format" module.
   The sub module: [labrat-it](labrat-it) is the "it" module.
...
#### Debugging ITs
  ...
  When running ITs, the Nexus Repository Manager will write log output to the following file:
      ...
      labrat/labrat-it/target/it-data/1/nexus3/log/nexus.log
